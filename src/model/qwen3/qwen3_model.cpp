...
Result<void> Qwen3Model::forward(const ForwardInput& input, ForwardOutput& output,
                                 ModelExecutionContext& exec_ctx) {
    Backend& backend = exec_ctx.backend;
    ...
    assert(exec_ctx.block_storage != nullptr);
    ...
    for (int l = 0; l < n_layers; ++l) {
...

        {
            ...
            auto k_cache = exec_ctx.block_storage->k_layer_tensor(l);
            auto v_cache = exec_ctx.block_storage->v_layer_tensor(l);
            auto r =
        ...
        {
            auto k_blocks = exec_ctx.block_storage->k_block_tensor(l);
            auto v_blocks = exec_ctx.block_storage->v_block_tensor(l);
