...
struct PhysicalBatch {
    ...
    std::shared_ptr<Buffer> positions;     // [num_tokens]
    std::shared_ptr<Buffer> slot_mapping;  // [num_tokens]

    int batch_size = 0;
    int max_blocks_per_req = 0;
    std::shared_ptr<Buffer> block_table;  // [batch_size, max_blocks_per_req]
    ...
    std::shared_ptr<Buffer> query_start_loc;  // [batch_size + 1]
    std::shared_ptr<Buffer> context_lens;     // [batch_size]
    ...
    std::shared_ptr<Buffer> logits_indices;  // [batch_size], int32
